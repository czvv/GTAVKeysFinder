    ...
    */
    ...
    public static class GTA5Constants
    {
        ...
        public static void Generate(byte[] exeData)
        {
            ...
            Console.WriteLine("NG Key found!");
            ...
            // Calculating NG Encryption Tables
            Console.WriteLine("Calculating NG Encryption Tables...");
            ...
            Console.WriteLine("Calculating NG Encryption Tables [1/17]");
            PC_NG_ENCRYPT_TABLES[0] = RandomGauss.Solve(PC_NG_DECRYPT_TABLES[0]);

            Console.WriteLine("Calculating NG Encryption Tables [2/17]");
            PC_NG_ENCRYPT_TABLES[1] = RandomGauss.Solve(PC_NG_DECRYPT_TABLES[1]);

            for (int k = 2; k <= 15; k++)
            {
                Console.WriteLine("Calculating NG Encryption Tables [" + (k + 1).ToString() + "/17]");
                PC_NG_ENCRYPT_LUTs[k] = LookUpTableGenerator.BuildLUTs2(PC_NG_DECRYPT_TABLES[k]);
            }

            Console.WriteLine("Calculating NG encryption tables [17/17]");
        ...
        public static void SaveToPath(string path)
        {
            ...
            CryptoIO.WriteNgTables(Path.Combine(path, "gtav_ng_encrypt_tables.dat"), PC_NG_ENCRYPT_TABLES);
            CryptoIO.WriteNgLuts(Path.Combine(path, "gtav_ng_encrypt_luts.dat"), PC_NG_ENCRYPT_LUTs);
            ...
            int outByte, int outBit)
        {
            ...
            for (int pivotIdx = 1; pivotIdx < 1024; pivotIdx++)
            {
                while (true)
                {
                    ...
                    // decrypt                   
                    var buf_decrypted = Decryption.DecryptNGRoundA(
                        buf_encrypted,
                        noKey,
                        tables);
